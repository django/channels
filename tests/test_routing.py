...
async def test_url_router():
    ...
    router = URLRouter(
        [
            url(r"^$", MockApplication(return_value=1)),
            url(r"^foo/$", MockApplication(return_value=2)),
            url(r"^bar", MockApplication(return_value=3)),
...
async def test_path_remaining():
    ...
    routes
    """
    inner_router = URLRouter([url(r"^no-match/$", MockApplication(return_value=1))])
